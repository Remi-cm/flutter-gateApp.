            ...
            onPressed: () async {
              if (!validateInputs()) {
                PaysmosmoAlert.showError(
                    context: context, message: 'Inuput fields cannot be empty');
              } else {
                LoadingDialog dialog =
                    LoadingDialog(context, LoadingDialogType.Normal);
                dialog.show();

                try {
                  dynamic response = await AuthService.registerUser(
                    userType: userTypeProvider.type,
